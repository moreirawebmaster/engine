...
class EngineBugTracking {
  ...
  static Future<void> log(final String message, {final String? level, final Map<String, dynamic>? attributes, final StackTrace? stackTrace}) async {
    if (isCrashlyticsEnabled) {
      await _crashlytics?.log(message);
    }

    if (isFaroEnabled) {
      await _faro?.pushLog(message, level: level, context: attributes, trace: {
        'stack': stackTrace?.toString() ?? 'Stack trace not available',
      });
    }
  }
  ...
  static Future<void> recordError(
    ...
    if (isCrashlyticsEnabled) {
      try {
        ...
        EngineLog.debug('Failed to record error in Crashlytics: $e');
      }
    }
  ...
  static Future<void> recordFlutterError(final FlutterErrorDetails errorDetails) async {
    if (isCrashlyticsEnabled) {
      try {
        ...
        EngineLog.debug('Failed to record Flutter error in Crashlytics: $e');
      }
    }
    ...
    if (kDebugMode) {
      if (isCrashlyticsEnabled) {
        try {
          ...
          EngineLog.debug('Failed to test crash in Crashlytics: $e');
        }
      }
    }
  }
